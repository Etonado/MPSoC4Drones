; ModuleID = '/home/erik/Documents/SDU/Courses/EmbSys/MPSoC4Drones/hls/KF_kernel/KF_kernel/solution1/.autopilot/db/a.g.ld.5.gdce.bc'
...
define void @apatb_KalmanFilterKernel_ir(float* %din, float* %dout, float %q, float %r, %struct.ap_uint* nocapture readonly %counter, i32 %set1) local_unnamed_addr #0 {
entry:
  ...
  call void @apatb_KalmanFilterKernel_hw(float* %2, float* %3, float %q, float %r, %struct.ap_uint* %counter, i32 %set1)
...
declare void @apatb_KalmanFilterKernel_hw(float*, float*, float, float, %struct.ap_uint*, i32)

define void @KalmanFilterKernel_hw_stub_wrapper(float*, float*, float, float, %struct.ap_uint*, i32) #5 {
entry:
  %6 = bitcast float* %0 to [6 x float]*
  %7 = bitcast float* %1 to [6 x float]*
  call void @copy_out([6 x float]* null, [6 x float]* %6, [6 x float]* null, [6 x float]* %7)
  %8 = bitcast [6 x float]* %6 to float*
  %9 = bitcast [6 x float]* %7 to float*
  call void @KalmanFilterKernel_hw_stub(float* %8, float* %9, float %2, float %3, %struct.ap_uint* %4, i32 %5)
  call void @copy_in([6 x float]* null, [6 x float]* %6, [6 x float]* null, [6 x float]* %7)
  ret void
}

declare void @KalmanFilterKernel_hw_stub(float*, float*, float, float, %struct.ap_uint*, i32)
